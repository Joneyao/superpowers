...
description: 在收到代码审查反馈时使用，在实施建议之前使用，特别是当反馈看起来不清晰或技术上有疑问时——需要技术严谨性和验证，而不是表演性认同或盲目实施
---
...
```
审查者："删除遗留代码"
✅ "检查中... 构建目标是 10.15+，这个 API 需要 13+。需要遗留代码来保持向后兼容。当前实现的 bundle ID 有误——修复它还是放弃 13 以下版本支持？"
```

**YAGNI（正确）：**
```
审查者："实现带数据库、日期过滤器、CSV 导出的完整指标跟踪"
✅ "在代码库中搜索了——没有任何地方调用这个端点。删除它（YAGNI）？还是有我遗漏的使用场景？"
```
